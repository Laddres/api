...
const sql = (idCargo, idEstado, genero, corRaca, primeiraCandidatura, idDispositivo) => {
...
const candidatosPorCargo = ({
  ...
  new Promise((resolve, reject) => {
    ...
    Promise.all([
      // Presidente
      db.query(sql(1, estado.id, genero, corRaca, primeiraCandidatura, idDispositivo)),
      // Governador
      db.query(sql(3, estado.id, genero, corRaca, primeiraCandidatura, idDispositivo)),
      // Senador
      db.query(sql(5, estado.id, genero, corRaca, primeiraCandidatura, idDispositivo)),
      // Deputado Federal
      db.query(sql(6, estado.id, genero, corRaca, primeiraCandidatura, idDispositivo)),
      // Deputado Estadual
      db.query(sql(7, estado.id, genero, corRaca, primeiraCandidatura, idDispositivo)),
    ])
      .then((resultados) => {
        const retorno = {
